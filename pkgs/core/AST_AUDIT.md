...
## Next AST work, in priority order
...
The outer command/control-flow tree is useful already, but several executable
sub-languages are still stored as text. Parsing all corpus files successfully
does not mean the AST is ready to evaluate without further parsing.
